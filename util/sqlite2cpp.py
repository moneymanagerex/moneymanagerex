...
''' % (self._table)
    ...
    bool ensure_data(wxSQLite3Database* db)
    {
        try
        {
            db->ExecuteQuery("SELECT * FROM %s"); // dummy code
        ''' % self._table

        for r in self._data:
            s += '''
            db->ExecuteUpdate(wxString::Format("INSERT INTO %s VALUES (%s)", %s));''' % (self._table, ', '.join(["'%s'" if isinstance(i, unicode) else str(i) for i in r]), ', '.join([ '_("' + i + '")' for i in r if isinstance(i, unicode)]))
