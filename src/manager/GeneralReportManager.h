...
public:
    ...
    sqlListCtrl(GeneralReportManager *grm, wxWindow *parent, const wxWindowID id);
    virtual wxString OnGetItemText(long item, long col_nr) const;
private:
    GeneralReportManager* m_grm;
};

class GeneralReportManager: public wxDialog
{
    wxDECLARE_DYNAMIC_CLASS(GeneralReportManager);
...
public:
    ...
    GeneralReportManager(wxWindow* parent, wxSQLite3Database* db, wxString itemname);
