...
wxIMPLEMENT_DYNAMIC_CLASS(PreferencesManager, wxDialog)

wxBEGIN_EVENT_TABLE( PreferencesManager, wxDialog )
    EVT_BUTTON(wxID_OK,                 PreferencesManager::OnOk)
    EVT_BUTTON(wxID_APPLY,              PreferencesManager::OnApply)
    EVT_LISTBOOK_PAGE_CHANGED(wxID_ANY, PreferencesManager::OnPageChange)
wxEND_EVENT_TABLE()
...
PreferencesManager::PreferencesManager(wxWindow* parent, mmGUIApp* app): m_app(app)
{
...
bool PreferencesManager::Create(wxWindow* parent
...
void PreferencesManager::CreateControls()
{
...
bool PreferencesManager::SaveNewSystemSettings()
{
    bool bResult = true;
...
void PreferencesManager::OnOk(wxCommandEvent& /*event*/)
{
    if(this->SaveNewSystemSettings())
        EndModal(wxID_OK);
}

void PreferencesManager::OnPageChange(wxBookCtrlEvent& event)
{
