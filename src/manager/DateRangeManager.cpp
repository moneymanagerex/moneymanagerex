...
#include "DateRangeManager.h"
#include "dialog/DateRangeDialog.h"

wxIMPLEMENT_DYNAMIC_CLASS(DateRangeManager, wxDialog);

wxBEGIN_EVENT_TABLE(DateRangeManager, wxDialog)
  EVT_BUTTON(wxID_OK,         DateRangeManager::OnOk)
  EVT_BUTTON(BTN_UP_TOP,      DateRangeManager::OnTop)
  EVT_BUTTON(BTN_UP,          DateRangeManager::OnUp)
  EVT_BUTTON(BTN_EDIT,        DateRangeManager::OnEdit)
  EVT_BUTTON(BTN_DOWN,        DateRangeManager::OnDown)
  EVT_BUTTON(BTN_DOWN_BOTTOM, DateRangeManager::OnBottom)
  EVT_BUTTON(BTN_NEW,         DateRangeManager::OnNew)
  EVT_BUTTON(BTN_DELETE,      DateRangeManager::OnDelete)
  EVT_BUTTON(BTN_DEFAULT,     DateRangeManager::OnDefault)
  EVT_DATAVIEW_SELECTION_CHANGED(wxID_ANY, DateRangeManager::OnDateRangeSelected)
wxEND_EVENT_TABLE()
...
DateRangeManager::DateRangeManager(
    wxWindow* parent,
    DateRangeManager::TYPE_ID type_id
) :
    m_type_id(type_id)
{
...
void DateRangeManager::fillControls()
{
...
void DateRangeManager::CreateControls()
{
...
void DateRangeManager::OnTop(wxCommandEvent&)
{
...
void DateRangeManager::OnUp(wxCommandEvent&)
{
...
void DateRangeManager::OnDown(wxCommandEvent&)
{
    exchangeItemData(m_selected_row, m_selected_row + 1);
    if (m_selected_row == m_date_range_m) {
        ++m_date_range_m;
...
void DateRangeManager::OnBottom(wxCommandEvent&)
{
    wxVector<wxVariant> data = getItemData(m_selected_row);
    m_dateRangesLb->DeleteItem(m_selected_row);
    m_dateRangesLb->UnselectRow(m_dateRangesLb->GetSelectedRow());
...
void DateRangeManager::OnNew(wxCommandEvent&)
{
    wxString name, range;
    DateRangeDialog dlg(this, &name, &range);
    if (dlg.ShowModal() == wxID_OK) {
...
void DateRangeManager::OnDelete(wxCommandEvent&)
{
...
void DateRangeManager::OnDateRangeSelected(wxDataViewEvent& event)
{
...
void DateRangeManager::updateButtonState(bool setselected) {
...
void DateRangeManager::OnOk(wxCommandEvent&)
{
    if (!m_hasChanged) {
...
void DateRangeManager::OnDefault(wxCommandEvent&)
{
    if (wxMessageBox(
...
// --- Support functions ---
wxVector<wxVariant> DateRangeManager::getItemData(int row) {
...
void DateRangeManager::exchangeItemData(int row1, int row2) {
...
void DateRangeManager::insertItemToLb(int pos, wxString name, wxString range)
{
