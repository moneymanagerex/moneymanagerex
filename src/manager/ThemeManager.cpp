...
wxIMPLEMENT_DYNAMIC_CLASS(ThemeManager, wxDialog);

wxBEGIN_EVENT_TABLE(ThemeManager, wxDialog)
    EVT_BUTTON(ID_DIALOG_THEME_IMPORT, ThemeManager::OnImport)
    EVT_BUTTON(ID_DIALOG_THEME_USE,    ThemeManager::OnUse)
    EVT_BUTTON(ID_DIALOG_THEME_DELETE, ThemeManager::OnDelete)
    EVT_BUTTON(wxID_OK,                ThemeManager::OnOk)
    EVT_LISTBOX(wxID_ANY,              ThemeManager::OnThemeView)
    EVT_HTML_LINK_CLICKED(wxID_ANY,    ThemeManager::OnHtmlLink)
wxEND_EVENT_TABLE()
...
bool ThemeManager::vfsThemeImageLoaded = false;

ThemeManager::ThemeEntry ThemeManager::getThemeEntry(const wxString& name)
{
...
void ThemeManager::addThemes(const wxString& themeDir, bool isSystem)
{
...
void ThemeManager::Create(wxWindow* parent, const wxString &name)
{
...
void ThemeManager::CreateControls()
{
...
void ThemeManager::OnThemeView(wxCommandEvent&)
{
    RefreshView();
}
...
void ThemeManager::OnOk(wxCommandEvent&)
{
    EndModal(wxID_OK);
}

void ThemeManager::OnHtmlLink(wxHtmlLinkEvent& event)
{
