...
struct DB_Table_SPLITTRANSACTIONS_V1 : public DB_Table
{
    ...
    bool ensure(wxSQLite3Database* db)
    {
        if (!exists(db))
		{
			try
			{
				db->ExecuteUpdate("CREATE TABLE SPLITTRANSACTIONS_V1(SPLITTRANSID integer primary key, TRANSID integer NOT nullptr, CATEGID integer, SUBCATEGID integer, SPLITTRANSAMOUNT numeric)");
			}
