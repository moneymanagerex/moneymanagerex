...
#pragma implementation "TrxFilterDialog.h"
#endif
...
wxIMPLEMENT_DYNAMIC_CLASS(TrxFilterDialog, wxDialog);

wxBEGIN_EVENT_TABLE(TrxFilterDialog, wxDialog)
    EVT_CHAR_HOOK(                  TrxFilterDialog::OnComboKey)
    EVT_CHECKBOX(wxID_ANY,          TrxFilterDialog::OnCheckboxClick)
    EVT_BUTTON(wxID_OK,             TrxFilterDialog::OnButtonOkClick)
    EVT_BUTTON(wxID_RESET,          TrxFilterDialog::OnButtonResetClick)
    EVT_BUTTON(wxID_CLEAR,          TrxFilterDialog::OnButtonClearClick)
    EVT_BUTTON(ID_BTN_CUSTOMFIELDS, TrxFilterDialog::OnMoreFields)
    EVT_MENU(wxID_ANY,              TrxFilterDialog::OnMenuSelected)
    EVT_DATE_CHANGED(wxID_ANY,      TrxFilterDialog::OnDateChanged)
    EVT_TIME_CHANGED(wxID_ANY,      TrxFilterDialog::OnDateChanged)
    EVT_CHOICE(wxID_ANY,            TrxFilterDialog::OnChoice)
    EVT_BUTTON(wxID_CANCEL,         TrxFilterDialog::OnButtonCancelClick)
    EVT_CLOSE(                      TrxFilterDialog::OnQuit)
wxEND_EVENT_TABLE()

TrxFilterDialog::TrxFilterDialog()
{
}
TrxFilterDialog::~TrxFilterDialog()
{
...
void TrxFilterDialog::mmDoInitVariables()
{
    ...
    if (j_tags.IsArray()) {
        for (rapidjson::SizeType i = 0; i < j_tags.Size(); i++) {
            if (j_tags[i].IsInt64()) {
                ...
                const TagData* tag_n = TagModel::instance().get_idN_data_n(int64(j_tags[i].GetInt64()));
                if (tag_n) {
...
void TrxFilterDialog::mmDoInitSettingNameChoice(wxString sel) const
{
    ...
    if (isMultiAccount_) {
        ...
        const AccountData* account_n = AccountModel::instance().get_idN_data_n(accountID_);
...
void TrxFilterDialog::mmDoCreateControls()
{
    ...
    *******************************************************************************/
    ...
    bSelectedAccounts_->Connect(wxID_ANY, wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(TrxFilterDialog::OnAccountsButton), nullptr, this);
    ...
    categoryComboBox_->Bind(wxEVT_KILL_FOCUS, &TrxFilterDialog::OnCategoryChange, this);
    ...
    *******************************************************************************/
    ...
    bHideColumns_->Connect(wxID_ANY, wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(TrxFilterDialog::OnShowColumnsButton), nullptr, this);
    ...
    *******************************************************************************/
    ...
    SetSizeHints(
        std::min({ best_size.GetWidth(), max_size.GetWidth() }),
        -1,
        max_size.GetWidth(),
        std::min({ max_size.GetHeight(), best_size.GetHeight() })
    );
    Fit();
    this->SetSizer(box_sizer);
    if (isReportMode_)
...
void TrxFilterDialog::OnCheckboxClick(wxCommandEvent& event)
{
    int id = event.GetId();
    switch (id)
    {
...
bool TrxFilterDialog::mmIsValuesCorrect() const
{
    ...
    if (m_use_date_filter && datesCheckBox_->IsChecked() &&
        rangeChoice_->GetSelection() == wxNOT_FOUND
    ) {
...
void TrxFilterDialog::OnButtonOkClick(wxCommandEvent& /*event*/)
{
...
void TrxFilterDialog::OnButtonCancelClick(wxCommandEvent& WXUNUSED(event))
{
#ifdef __WXMSW__
...
void TrxFilterDialog::OnQuit(wxCloseEvent& /*event*/)
{
    EndModal(wxID_CANCEL);
}
...
bool TrxFilterDialog::mmIsSomethingChecked() const
{
...
bool TrxFilterDialog::mmIsNoteMatches(const wxString& note)
{
...
bool TrxFilterDialog::mmIsCategoryMatches(int64 categid)
{
...
bool TrxFilterDialog::mmIsTagMatches(RefTypeN ref_type, int64 ref_id, bool mergeSplitTags)
{
    ...
    if (ref_type == TrxSplitModel::s_ref_type)
        ...
        tag_name_id_m = TagLinkModel::instance().find_ref_mTagName(
            ...
            SchedSplitModel::instance().get_idN_data_n(ref_id)->m_sched_id
        );
...
const wxString TrxFilterDialog::mmGetDescriptionToolTip() const
{
    wxString buffer;
        ...
        case kArrayType:
        {
            ...
            for (unsigned int i = 0; i < valArray.Size(); i++)
            {
                if (valArray[i].GetType() == kNumberType)
                {
                    ...
                    {
                        value += (value.empty() ? "" : " ") + TagModel::instance().get_idN_data_n(int64(valArray[i].GetInt64()))->m_name;
...
void TrxFilterDialog::mmGetDescription(mmHTMLBuilder& hb)
{
        ...
        case kArrayType:
        {
            ...
            for (const auto& a : itr->value.GetArray())
            {
                if (a.GetType() == kNumberType)
                {
                    ...
                    {
                        temp += (temp.empty() ? "" : (appendOperator ? " & " : " ")) + TagModel::instance().get_idN_data_n(int64(a.GetInt64()))->m_name;
                        appendOperator = true;
                    }
...
const wxString TrxFilterDialog::mmGetJsonSettings(bool i18n) const
{
    ...
    if (cf.size() > 0) {
        for (const auto& i : cf) {
            if (!i.second.empty()) {
                const auto field_n = FieldModel::instance().get_idN_data_n(i.first);
...
void TrxFilterDialog::OnCategoryChange(wxEvent& event)
{
        ...
        if (pattern.IsValid()) {
            for (const auto& cat_fullname_id : CategoryModel::instance().find_all_id_mFullname()) {
                ...
                if (!mmIsCategorySubCatChecked())
                    continue;
                const CategoryData* cat_n = CategoryModel::instance().get_idN_data_n(cat_fullname_id.second);
...
void TrxFilterDialog::OnDateChanged(wxDateEvent& event)
{
    switch (event.GetId())
    {
    case wxID_FIRST:
...
void TrxFilterDialog::mmDoResetFilterStatus()
{
    // m_custom_fields->ResetWidgetsChanged();
}

void TrxFilterDialog::OnSettingsSelected(wxCommandEvent& event)
{
    if (!isMultiAccount_)
        return;
...
void TrxFilterDialog::mmDoSaveSettings(bool is_user_request)
{
...
void TrxFilterDialog::OnAccountsButton(wxCommandEvent& WXUNUSED(event))
{
    ...
    if (m_selected_accounts_id.empty()) {
        ...
        const AccountData* account_n = AccountModel::instance().get_idN_data_n(*m_selected_accounts_id.begin());
        if (account_n)
...
void TrxFilterDialog::OnMoreFields(wxCommandEvent& WXUNUSED(event))
{
...
void TrxFilterDialog::OnChoice(wxCommandEvent& event)
{
    int id = event.GetId();
