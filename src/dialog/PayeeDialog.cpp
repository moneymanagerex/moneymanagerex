...
wxIMPLEMENT_DYNAMIC_CLASS(PayeeDialog, wxDialog);

wxBEGIN_EVENT_TABLE(PayeeDialog, wxDialog)
    EVT_CHAR_HOOK(PayeeDialog::OnComboKey)
    EVT_BUTTON(wxID_CANCEL, PayeeDialog::OnCancel)
    EVT_BUTTON(wxID_OK, PayeeDialog::OnOk)
    EVT_BUTTON(wxID_UP, PayeeDialog::OnMoveUp)
    EVT_BUTTON(wxID_DOWN, PayeeDialog::OnMoveDown)
    EVT_GRID_CELL_CHANGED(PayeeDialog::OnPatternTableChanged)
wxEND_EVENT_TABLE()

PayeeDialog::PayeeDialog()
{
}
PayeeDialog::PayeeDialog(wxWindow *parent, PayeeModel::Data* payee, const wxString &name) :
m_payee(payee)
{
...
PayeeDialog::~PayeeDialog()
{
    ...
    m_patternTable->GetGridWindow()->Unbind(wxEVT_SIZE, &PayeeDialog::OnPatternTableSize, this);
}

void PayeeDialog::CreateControls()
{
    ...
    m_patternTable->GetGridWindow()->Bind(wxEVT_SIZE, &PayeeDialog::OnPatternTableSize, this);
...
void PayeeDialog::OnOk(wxCommandEvent& /*event*/)
{
...
void PayeeDialog::OnCancel(wxCommandEvent& /*event*/)
{
    EndModal(wxID_OK);
}

void PayeeDialog::OnMoveUp(wxCommandEvent& /*event*/)
{
    // ignore the last row
...
void PayeeDialog::OnMoveDown(wxCommandEvent& /*event*/)
{
    // ignore the last row
    m_patternTable->DeselectRow(m_patternTable->GetNumberRows() - 1);
...
void PayeeDialog::OnComboKey(wxKeyEvent& event)
{
...
void PayeeDialog::OnPatternTableChanged(wxGridEvent& event)
{
...
void PayeeDialog::OnPatternTableSize(wxSizeEvent&)
{
    m_patternTable->SetColSize(0, m_patternTable->GetGridWindow()->GetSize().x);
}

void PayeeDialog::ResizeDialog()
{
...
void mmPayeeDialog::DefineDefaultCategory()
{
    ...
    if (nb > 0) {
        ...
        CategoryDialog dlg(this, true, nb == 1 ? payee->CATEGID : -1);
        if (dlg.ShowModal() == wxID_OK) {
            for(RowData* rdata : m_selectedItems) {
        ...
        CategoryDialog dlg(this, false, -1);  // show category dialog only
        dlg.ShowModal();
    }
}
