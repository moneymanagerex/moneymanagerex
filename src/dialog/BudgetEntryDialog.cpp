...
wxBEGIN_EVENT_TABLE( BudgetEntryDialog, wxDialog )
    EVT_BUTTON(wxID_OK, BudgetEntryDialog::OnOk)
    EVT_TEXT_ENTER(wxID_ANY, BudgetEntryDialog::OnOk)
wxEND_EVENT_TABLE()
...
{    
    ...
    const CategoryModel::Data* category = CategoryModel::instance().cache_id(budgetEntry_->CATEGID);
    wxASSERT(category);
    ...
    m_choiceItem->Connect(wxID_ANY, wxEVT_CHAR, wxKeyEventHandler(BudgetEntryDialog::onChoiceChar), nullptr, this);
