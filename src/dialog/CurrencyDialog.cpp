...
wxIMPLEMENT_DYNAMIC_CLASS(CurrencyDialog, wxDialog);

wxBEGIN_EVENT_TABLE(CurrencyDialog, wxDialog)
EVT_BUTTON(wxID_OK, CurrencyDialog::OnOk)
EVT_BUTTON(wxID_CANCEL, CurrencyDialog::OnCancel)
EVT_TEXT(ID_DIALOG_CURRENCY, CurrencyDialog::OnDataChanged)
EVT_CHOICE(ID_DIALOG_CURRENCY, CurrencyDialog::OnDataChanged)
EVT_RADIOBUTTON(ID_DIALOG_CURRENCY, CurrencyDialog::OnDataChanged)
wxEND_EVENT_TABLE()
...
CurrencyDialog::CurrencyDialog(wxWindow* parent, const CurrencyModel::Data * currency)
    : m_scale(SCALE)
{
    if (currency)
    {
        m_currency = CurrencyModel::instance().clone(currency);
        m_currency->CURRENCYID = currency->CURRENCYID;
        m_currency->BASECONVRATE = CurrencyHistoryModel::getLastRate(m_currency->CURRENCYID);
    }
    else
    {
        m_currency = CurrencyModel::instance().create();
        m_currency->BASECONVRATE = 1;
        m_currency->SCALE = 100;
        m_currency->DECIMAL_POINT = ".";
        m_currency->GROUP_SEPARATOR = ",";
        m_currency->CURRENCY_TYPE = CurrencyModel::TYPE_NAME_FIAT;
    }
...
bool CurrencyDialog::Create(wxWindow* parent, wxWindowID id
...
void CurrencyDialog::fillControls()
{
    if (m_currency)
    {
        ...
        bool baseCurrency = (PreferencesModel::instance().getBaseCurrencyID() == m_currency->CURRENCYID);
...
void CurrencyDialog::CreateControls()
{
    ...
    mctrl_baseConvRate->Connect(ID_DIALOG_CURRENCY_RATE, wxEVT_COMMAND_TEXT_ENTER
        , wxCommandEventHandler(CurrencyDialog::OnTextEntered), nullptr, this);
...
void CurrencyDialog::OnOk(wxCommandEvent& WXUNUSED(event))
{
    ...
    const auto currency_code = CurrencyModel::instance().find(CurrencyModel::CURRENCY_SYMBOL(code));
    ...
    CurrencyModel::instance().save(m_currency);
    EndModal(wxID_OK);
}

void CurrencyDialog::OnCancel(wxCommandEvent& /*event*/)
{
    EndModal(wxID_CANCEL);
}

void CurrencyDialog::OnDataChanged(wxCommandEvent& WXUNUSED(event))
{
    ...
    dispAmount = wxString::Format(_t("%.2f Shown As: %s"), base_amount, CurrencyModel::toCurrency(base_amount, m_currency, scale));
    if (m_locale_used)
        dispAmount = dispAmount + "  " + _t("(Using Locale)");
    mctrl_sampleText->SetLabelText(dispAmount);
}

void CurrencyDialog::OnTextEntered(wxCommandEvent& WXUNUSED(event))
{
    if (mctrl_baseConvRate->Calculate(SCALE))
        mctrl_baseConvRate->GetDouble(m_currency->BASECONVRATE);
}
