...
wxBEGIN_EVENT_TABLE( BudgetYearDialog, wxDialog )
    EVT_BUTTON(wxID_OK, BudgetYearDialog::OnOk)
...
BudgetYearDialog::BudgetYearDialog(wxWindow* parent)
{
    budgetYearID_ = -1;
...
bool BudgetYearDialog::Create(wxWindow* parent, wxWindowID id,
...
{
    int index = 0;
    for (const auto& e: BudgetPeriodModel::instance().get_all(BudgetPeriodModel::COL_BUDGETYEARNAME))
    {
...
void BudgetYearDialog::CreateControls()
{    
...
void BudgetYearDialog::OnAdd(wxCommandEvent& /*event*/)
{
    BudgetYearEntryDialog dlg(this); 
...
void BudgetYearDialog::OnAddMonth(wxCommandEvent& /*event*/)
{
    BudgetYearEntryDialog dlg(this, true); 
    if ( dlg.ShowModal() == wxID_OK )
    {
        m_listBox->Clear();
        fillControls();
