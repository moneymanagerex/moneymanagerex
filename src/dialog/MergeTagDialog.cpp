...
wxIMPLEMENT_DYNAMIC_CLASS(MergeTagDialog, wxDialog);

wxBEGIN_EVENT_TABLE(MergeTagDialog, wxDialog)
    EVT_CHILD_FOCUS(MergeTagDialog::OnFocusChange)
    EVT_BUTTON(wxID_OK, MergeTagDialog::OnOk)
    EVT_BUTTON(wxID_CANCEL, MergeTagDialog::OnCancel)
    EVT_COMBOBOX(wxID_ANY, MergeTagDialog::OnTextUpdated)
wxEND_EVENT_TABLE()
...
bool MergeTagDialog::Create(wxWindow* parent
...
void MergeTagDialog::CreateControls()
{
...
void MergeTagDialog::OnCancel(wxCommandEvent& WXUNUSED(event))
{
    EndModal(m_changed_records > 0 ? wxID_OK : wxID_CANCEL);
}

void MergeTagDialog::OnOk(wxCommandEvent& WXUNUSED(event))
{
...
void MergeTagDialog::OnFocusChange(wxChildFocusEvent& event)
{
    IsOkOk();
    event.Skip();
}

void MergeTagDialog::OnTextUpdated(wxCommandEvent& )
{
    IsOkOk();
}
