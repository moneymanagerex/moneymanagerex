...
EVT_BUTTON(wxID_OK, CategoryDialog::OnBSelect)
EVT_BUTTON(wxID_CANCEL, CategoryDialog::OnCancel)
EVT_BUTTON(wxID_ADD, CategoryDialog::OnAdd)
EVT_BUTTON(wxID_REMOVE, CategoryDialog::OnDelete)
EVT_BUTTON(wxID_EDIT, CategoryDialog::OnEdit)
EVT_TOGGLEBUTTON(wxID_SELECTALL, CategoryDialog::OnShowHiddenToggle)
EVT_TEXT(wxID_FIND, CategoryDialog::OnTextChanged)
EVT_TREE_SEL_CHANGED(wxID_ANY, CategoryDialog::OnSelChanged)
EVT_TREE_ITEM_RIGHT_CLICK(wxID_ANY, CategoryDialog::OnSelChanged)
EVT_TREE_ITEM_ACTIVATED(wxID_ANY, CategoryDialog::OnDoubleClicked)
EVT_TREE_ITEM_MENU(wxID_ANY, CategoryDialog::OnItemRightClick)
EVT_TREE_ITEM_COLLAPSED(wxID_ANY, CategoryDialog::OnItemCollapseOrExpand)
EVT_TREE_ITEM_EXPANDED(wxID_ANY, CategoryDialog::OnItemCollapseOrExpand)
EVT_TREE_BEGIN_DRAG(wxID_ANY, CategoryDialog::OnBeginDrag)
EVT_TREE_END_DRAG(wxID_ANY, CategoryDialog::OnEndDrag)
EVT_MENU(wxID_ANY, CategoryDialog::OnMenuSelected)
wxEND_EVENT_TABLE()
...
bool CategoryDialog::Create(wxWindow* parent, wxWindowID id
...
void CategoryDialog::saveCurrentCollapseState()
{
...
bool CategoryDialog::AppendSubcategoryItems(wxTreeItemId parent, const CategoryModel::Data* category) {
...
void CategoryDialog::CreateControls()
{
    ...
    m_buttonRelocate->Connect(wxID_REPLACE_ALL, wxEVT_COMMAND_BUTTON_CLICKED
        , wxCommandEventHandler(CategoryDialog::OnCategoryRelocation), nullptr, this);
    ...
    m_tbCollapse->Connect(ID_COLLAPSE, wxEVT_BUTTON,
        wxCommandEventHandler(CategoryDialog::OnExpandOrCollapseToggle), nullptr, this);
    ...
    clearBtn->Connect(ID_CLEAR, wxEVT_BUTTON,
        wxCommandEventHandler(CategoryDialog::OnClearSettings), nullptr, this);
...
bool CategoryDialog::validateName(wxString name)
{
...
void CategoryDialog::OnAdd(wxCommandEvent& /*event*/)
{
...
void CategoryDialog::OnBeginDrag(wxTreeEvent& event)
{
...
void CategoryDialog::OnEndDrag(wxTreeEvent& event)
{
...
void CategoryDialog::showCategDialogDeleteError(bool category)
{
...
void CategoryDialog::mmDoDeleteSelectedCategory()
{
...
void CategoryDialog::OnDelete(wxCommandEvent& /*event*/)
{
...
void CategoryDialog::OnBSelect(wxCommandEvent& /*event*/)
{
    if (m_selectedItemId != root_ && m_selectedItemId)
        EndModal(wxID_OK);
}

void CategoryDialog::OnDoubleClicked(wxTreeEvent& /*event*/)
{
...
void CategoryDialog::OnSelChanged(wxTreeEvent& event)
{
...
void CategoryDialog::OnEdit(wxCommandEvent& /*event*/)
{
    if (m_selectedItemId == root_ || !m_selectedItemId)
        return;
...
wxTreeItemId CategoryDialog::getTreeItemFor(const wxTreeItemId& itemID, const wxString& itemText, const int64 parentid)
{
    wxTreeItemIdValue treeDummyValue;
...
void CategoryDialog::setTreeSelection(int64 category_id)
{
...
void CategoryDialog::setTreeSelection(const wxString& catName, const int64 parentid)
{
    if (!catName.IsEmpty())
    {
...
void CategoryDialog::OnCategoryRelocation(wxCommandEvent& /*event*/)
{
...
void CategoryDialog::OnExpandOrCollapseToggle(wxCommandEvent& event)
{
    m_processExpandCollapse = false;
...
void CategoryDialog::OnShowHiddenToggle(wxCommandEvent& /*event*/)
{
    SettingModel::instance().setBool("SHOW_HIDDEN_CATEGS", m_tbShowAll->GetValue());
    fillControls();
}

void CategoryDialog::OnTextChanged(wxCommandEvent& event)
{
...
void CategoryDialog::OnMenuSelected(wxCommandEvent& event)
{
    int id = event.GetId();
...
void CategoryDialog::OnClearSettings(wxCommandEvent& /*event*/)
{
...
void CategoryDialog::OnItemRightClick(wxTreeEvent& event)
{
    wxMenu mainMenu;
...
void CategoryDialog::OnItemCollapseOrExpand(wxTreeEvent& event)
{
...
bool CategoryDialog::categShowStatus(int64 categId)
{
...
wxString CategoryDialog::getFullCategName()
{
    return CategoryModel::full_name(m_categ_id);
}

bool CategoryDialog::mmIsUsed() const
{
