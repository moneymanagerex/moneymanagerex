...
wxBEGIN_EVENT_TABLE(AttachmentPreferences, wxPanel)
    EVT_BUTTON(ID_DIALOG_OPTIONS_BUTTON_ATTACHMENTSFOLDER, AttachmentPreferences::OnAttachmentsButton)
    EVT_TEXT(ID_DIALOG_OPTIONS_TEXTCTRL_ATTACHMENT, AttachmentPreferences::OnAttachmentsPathChanged)
    EVT_CHECKBOX(ID_DIALOG_OPTIONS_CHECKBOX_ATTACHMENTSSUBFOLDER, AttachmentPreferences::OnAttachmentsSubfolderChanged)
wxEND_EVENT_TABLE()
/*******************************************************/
...
AttachmentPreferences::AttachmentPreferences(wxWindow *parent
    , wxWindowID id
    , const wxPoint &pos
    , const wxSize &size
...
void AttachmentPreferences::Create()
{
    ...
    wxStaticText* attachmentStaticText = new wxStaticText(attachmentStaticBox, wxID_STATIC, attachmentStaticText_desc);
    ...
    const wxString attachmentFolder = InfotableModel::instance().getString(
    ...
    m_attachments_path = new wxComboBox(attachmentStaticBox, ID_DIALOG_OPTIONS_TEXTCTRL_ATTACHMENT, "", wxDefaultPosition, wxDefaultSize, list);
    m_attachments_path->SetMinSize(wxSize(225, -1));
    m_attachments_path->ChangeValue(attachmentFolder);

    wxButton* AttachmentsFolderButton = new wxButton(attachmentStaticBox, ID_DIALOG_OPTIONS_BUTTON_ATTACHMENTSFOLDER, "..."
    ...
    // Legend
    wxStaticBox* attachmentStaticBoxLegend = new wxStaticBox(attachmentStaticBox, wxID_ANY, _t("Legend (can be used as variables at the beginning of above path)"));
    ...
    wxStaticText* legendStaticText = new wxStaticText(attachmentStaticBoxLegend, wxID_STATIC, legend);
    attachmentStaticBoxSizerLegend->Add(legendStaticText);
    //End legend

    //Other OS folders
    wxStaticBox* attachmentStaticBoxInfo = new wxStaticBox(attachmentStaticBox, wxID_ANY, _t("Other OS folders "));
    ...
    const wxString attachmentFolderWin = InfotableModel::instance().getString(
        "ATTACHMENTSFOLDER:Win", FolderNotSet
    );
    const wxString attachmentFolderMac = InfotableModel::instance().getString(
        "ATTACHMENTSFOLDER:Mac", FolderNotSet
    );
    const wxString attachmentFolderUnix = InfotableModel::instance().getString(
    ...
    m_attachments_subfolder = new wxCheckBox(attachmentStaticBox, ID_DIALOG_OPTIONS_CHECKBOX_ATTACHMENTSSUBFOLDER
        , cbAttachmentsSubfolder_desc, wxDefaultPosition, wxDefaultSize, wxCHK_2STATE);
    m_attachments_subfolder->SetValue(InfotableModel::instance().getBool("ATTACHMENTSSUBFOLDER", true));
    attachmentStaticBoxSizer->Add(m_attachments_subfolder, g_flagsV);
    attachmentStaticBoxSizer->Add(new wxStaticText(attachmentStaticBox, wxID_STATIC, subFolder), g_flagsV);
    ...
    m_delete_attachments->SetValue(InfotableModel::instance().getBool("ATTACHMENTSDELETE", false));
    ...
    m_trash_attachments->SetValue(InfotableModel::instance().getBool("ATTACHMENTSTRASH", false));
    ...
    m_duplicate_attachments->SetValue(InfotableModel::instance().getBool("ATTACHMENTSDUPLICATE", false));
    ...
    SetBoldFontToStaticBoxHeader(attachmentStaticBox);
    Fit();
    attachment_panel->SetMinSize(attachment_panel->GetBestVirtualSize());
    attachment_panel->SetScrollRate(6, 6);
}

void AttachmentPreferences::OnAttachmentsButton(wxCommandEvent& WXUNUSED(event))
{
...
void AttachmentPreferences::OnAttachmentsPathChanged(wxCommandEvent& WXUNUSED(event))
{
...
void AttachmentPreferences::OnAttachmentsSubfolderChanged(wxCommandEvent& event)
{
    InfotableModel::instance().setBool(
...
bool AttachmentPreferences::SaveSettings()
{
    ...
    InfotableModel::instance().setBool("ATTACHMENTSDELETE", m_delete_attachments->GetValue());
    InfotableModel::instance().setBool("ATTACHMENTSTRASH", m_trash_attachments->GetValue());
    InfotableModel::instance().setBool("ATTACHMENTSDUPLICATE", m_duplicate_attachments->GetValue());
