...
void GeneralPref::Create()
{
    ...
    const CurrencyData* currency = CurrencyModel::instance().get_id_data_n(PrefModel::instance().getBaseCurrencyID());
    ...
    { // Locale
        ...
        m_itemListOfLocales->Connect(ID_DIALOG_OPTIONS_LOCALE, wxEVT_COMMAND_TEXT_UPDATED
            , wxCommandEventHandler(GeneralPref::OnLocaleChanged), nullptr, this);
        m_itemListOfLocales->SetMinSize(wxSize(100, -1));
    }
    ...
    m_currency_history->SetValue(PrefModel::instance().getUseCurrencyHistory());
