...
wxBEGIN_EVENT_TABLE(ListBase, wxListCtrl)
    ...
    EVT_LIST_COL_CLICK(wxID_ANY,       ListBase::onColClick)
    EVT_LIST_COL_RIGHT_CLICK(wxID_ANY, ListBase::onColRightClick)
    EVT_MENU(wxID_ANY,                 ListBase::onHeaderPopup)
wxEND_EVENT_TABLE()
...
ListBase::ListBase(wxWindow* parent_win, wxWindowID win_id) :
    wxListCtrl(
        parent_win, win_id, wxDefaultPosition, wxDefaultSize,
...
wxString ListBase::buildPage(const wxString &title) const
{
    ...
    text << "<!DOCTYPE html>" << eol
        ...
        << ((GetWindowStyle() & wxLC_HRULES) || (GetWindowStyle() & wxLC_VRULES) ? "1" : "0")
            << " cellpadding=4 cellspacing=0 >" << eol
        << "<tr>" << eol;
...
const wxString ListBase::getColHeader(int col_id, bool show_icon) const
{
    ListColumnInfo col_info = m_col_info_id[col_id];
...
void ListBase::loadPref_v190()
{
    // load m_col_id_nr if columns can be ordered
    if (!m_col_id_nr.empty() && !o_col_order_prefix.empty()) {
        wxString order_str = SettingModel::instance().getString(getColOrderKey_v190(), "");
    ...
    for (int col_id = 0; col_id < getColIdSize(); ++col_id) {
        ...
        if (!o_col_width_prefix.empty()) {
            col_width = SettingModel::instance().getInt(
    ...
    // load m_sort_col_id, m_sort_asc
    // NOTE:
    //   *_SORT_COL* represents a col_nr in JournalList (others have a bug).
    //   *_ASC* is stored as Int in JournalList, or as Bool otherwise.
    //   Only JournalList has two sorting columns.
    bool asc_isInt = (m_sort_col_id.size() == 2);
    if (c_sort_col_nr.size() != m_sort_col_id.size())
        c_sort_col_nr = std::vector<int>(m_sort_col_id.size(), -1);
    if (m_sort_asc.size() != m_sort_col_id.size())
        m_sort_asc = std::vector<bool>(m_sort_col_id.size(), true);
    for (int i = 0; i < static_cast<int>(m_sort_col_id.size()); ++i) {
        int col_nr = SettingModel::instance().getInt(getSortColKey_v190(i), -1);
...
{
    return isInt ?
        SettingModel::instance().getInt(key, (default_value ? 1 : 0)) != 0 :
        SettingModel::instance().getBool(key, default_value);
}

//----------------------------------------------------------------------------

int ListBase::cacheSortColNr(int i)
{
...
wxListItemAttr* ListBase::OnGetItemAttr(long row) const
{
    return (row % 2) ? w_attr2.get() : w_attr1.get();
}

void ListBase::onColClick(wxListEvent& WXUNUSED(event))
{
    // To be implemented in derived class
}

int ListBase::getSortIcon(bool WXUNUSED(asc)) const
{
    return -1;
}

void ListBase::updateSortIcon()
{
...
void ListBase::shiftColumn(int col_vo, int offset)
{
    ...
    wxLogDebug("ListBase::shiftColumn(): save column %d:%d (%s)",
    ...
    while (offset != 0) {
        ...
        wxLogDebug("ListBase::shiftColumn(): move column %d:%d (%s) -> %d:%d",
    ...
    wxLogDebug("ListBase::shiftColumn(): restore column (%s) -> %d:%d",
...
void ListBase::onColRightClick(wxListEvent& event)
{
...
void ListBase::onHeaderPopup(wxCommandEvent& event)
{
...
void ListBase::onHeaderSort(wxCommandEvent& WXUNUSED(event))
{
    wxListEvent e;
...
void ListBase::onHeaderToggle(wxCommandEvent& event)
{
...
void ListBase::onHeaderHide(wxCommandEvent& WXUNUSED(event))
{
    if (!isValidColNr(m_sel_col_nr))
        return;
    Freeze();
...
void ListBase::onHeaderShow(wxCommandEvent& event)
{
...
void ListBase::onHeaderMove(wxCommandEvent& WXUNUSED(event), int dir)
{
    if (m_col_id_nr.empty())
        return;
    Freeze();
...
void ListBase::headerMoveBeginEnd(bool dir){
    if (!m_col_id_nr.empty()) {
        Freeze();
        int src_nr = m_sel_col_nr;
        int src_vo = getColVo_Nr(src_nr);
...
void ListBase::onHeaderReset(wxCommandEvent& WXUNUSED(event))
{
    Freeze();
    m_col_hidden_id.clear();
    for (int col_nr = 0; col_nr < getColNrSize(); ++col_nr) {
