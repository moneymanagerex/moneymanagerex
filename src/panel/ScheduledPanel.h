...
class ScheduledList: public ListBase
{
    DECLARE_NO_COPY_CLASS(ScheduledList)
...
private:
    ...
    ScheduledPanel* m_bdp;
    long m_selected_row = -1;

public:
    ScheduledList(ScheduledPanel* bdp, wxWindow *parent, wxWindowID winid = wxID_ANY);
    ~ScheduledList();
...
class ScheduledPanel : public PanelBase
{
    wxDECLARE_EVENT_TABLE();
...
public:
    ...
    ScheduledPanel(wxWindow *parent
        , wxWindowID winid = wxID_ANY
        , const wxPoint& pos = wxDefaultPosition
        , const wxSize& size = wxDefaultSize
        , long style = wxTAB_TRAVERSAL | wxNO_BORDER
        , const wxString& name = "ScheduledPanel" 
    );
    ~ScheduledPanel();
...
inline int ScheduledList::col_sort() { return LIST_ID_PAYMENT_DATE; }
