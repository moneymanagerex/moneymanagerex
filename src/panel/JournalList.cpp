...
void JournalList::refreshVisualList(bool filter)
{
    ...
    // Grab the selected transactions unless we have freshly pasted transactions
    // in which case use them
...
void JournalList::onMarkTrx(wxCommandEvent& event)
{
    ...
    switch (evt)
    {
    case MENU_TREEPOPUP_MARKRECONCILED:
        status = TrxStatus(TrxStatus::e_reconciled);
        break;
    case MENU_TREEPOPUP_MARKUNRECONCILED:
        status = TrxStatus(TrxStatus::e_unreconciled);
        break;
    case MENU_TREEPOPUP_MARKVOID:
        status = TrxStatus(TrxStatus::e_void);
        break;
    case MENU_TREEPOPUP_MARK_ADD_FLAG_FOLLOWUP:
        status = TrxStatus(TrxStatus::e_followup);
        break;
    case MENU_TREEPOPUP_MARKDUPLICATE:
        status = TrxStatus(TrxStatus::e_duplicate);
        break;
    default:
        wxASSERT(false);
    }
