    ...
    , const wxString& link_type, const int64 link_record_id)
{
    ...
    */
    TransactionModel::Data* checking_entry = TransactionModel::instance().get(checking_id);
...
void TransactionLinkModel::RemoveTranslinkEntry(const int64 checking_account_id)
{
    ...
    {
        AssetModel::Data* asset_entry = AssetModel::instance().get(translink.LINKRECORDID);
        UpdateAssetValue(asset_entry);
    }

    if (translink.LINKTYPE == StockModel::refTypeName)
    {
        StockModel::Data* stock_entry = StockModel::instance().get(translink.LINKRECORDID);
...
void TransactionLinkModel::UpdateAssetValue(AssetModel::Data* asset_entry)
{
    ...
    {
        TransactionModel::Data* asset_trans = TransactionModel::instance().get(trans.CHECKINGACCOUNTID);
        ...
        {
            CurrencyModel::Data* asset_currency = AccountModel::currency(AccountModel::instance().get(asset_trans->ACCOUNTID));
...
bool TransactionLinkModel::ShareAccountId(int64& stock_entry_id)
{
    ...
    if (!stock_translink_list.empty())
    {
        ...
        {
            AccountModel::Data* account_entry = AccountModel::instance().get(checking_entry.at(0).ACCOUNTID);
