...
wxString mmReportSummaryByDate::getHTMLText()
{
    ...
    for (const auto & dd : arDates)
    {
        ...
        totBalanceData.push_back(Model_Currency::toCurrency(balancePerDay[Model_Account::CRYPTO_WALLET]));
        total = balancePerDay[Model_Account::CASH] + balancePerDay[Model_Account::CHECKING] + balancePerDay[Model_Account::CREDIT_CARD]
            + balancePerDay[Model_Account::LOAN] + balancePerDay[Model_Account::TERM] + +balancePerDay[Model_Account::CRYPTO_WALLET];
