...
void mmHTMLBuilder::addTotalRow(const wxString& caption, int cols, const std::vector<double>& data)
{
    ...
    {
        if (value) data_str.push_back(Model_Currency::toCurrency(value));
        else data_str.push_back("");
    }
...
void mmHTMLBuilder::addPieChart(std::vector<ValueTrio>& valueList)
{
    ...
    static const wxString js = "<script type='text/javascript'>\n"
        ...
        "</script>\n";
    html_ += tags::TABLE_ROW;
    this->addTableCell("<canvas id='reportChart' width ='300' height='300'></canvas>");
    this->addText(wxString::Format(js, data));
    this->endTableCell();
}
