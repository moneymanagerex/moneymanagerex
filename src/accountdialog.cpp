...
void mmNewAcctDialog::fillControls()
{
    ...
    if (!m_account->INITIALDATE.empty()) {
        m_initdate_ctrl->SetValue(isoDateTime(m_account->INITIALDATE));
    }
    ...
    if (!m_account->PAYMENTDUEDATE.empty()) {
        m_payment_due_date_ctrl->SetValue(isoDateTime(m_account->PAYMENTDUEDATE));
    }
    ...
    if (!m_account->STATEMENTDATE.empty()) {
        m_statement_date_ctrl->SetValue(isoDateTime(m_account->STATEMENTDATE));
    }
