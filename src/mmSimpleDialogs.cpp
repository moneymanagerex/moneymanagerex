    ...
    : wxPanel(), operatorAllowed_(operatorAllowed)
{
    ...
    Bind(wxEVT_CHILD_FOCUS, &mmTagTextCtrl::OnSetFocus, this);
    h_sizer->Add(textCtrl_, 1, wxALIGN_CENTER_VERTICAL | wxLEFT, 5);
...
void mmTagTextCtrl::init()
{
    ...
    tagCheckListBox_->GetParent()->Fit();
    popupWindow_->Fit();
    initialized_ = true;
}
...
void mmTagTextCtrl::OnSetFocus(wxChildFocusEvent& event) {
    if (!initialized_)
        init();
}
