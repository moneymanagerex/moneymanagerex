...
bool mmGUIFrame::createDataStore(const wxString& fileName, const wxString& pwd, bool openingNew)
{
        ...
        && passwordCheckPassed)
    {
        ...
        {
            const wxString note = wxString::Format("%1$s - %2$s", mmex::getProgramName(), _("No File opened"));
            this->SetTitle(note);
...
{
    wxString title = wxString::Format("%1$s - %2$s", mmex::getProgramName(), fileName);
    if (mmex::isPortableMode())
        title = wxString::Format("%1$s - %2$s", title, (" [" + _("portable mode") + ']'));
