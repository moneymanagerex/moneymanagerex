...
//
// ***** 12 January 2014 - Modified by James Higley *****
// Changed mg_destroy_server to fixed memory leaks found when embedded in mmex.
// Fixes for mmex compile messages for Windows 32-bit build. The Windows 64-bit build does
// have some C4244 and C4267 compiler warnings (conversion from ... possible loss of data).
//
...
// Modification for mmex compiler warning (only define if not already defined)
...
// Modification for mmex compiler warning (disable 4100: unreferenced formal parameter)
...
static int mg_socketpair(sock_t sp[2]) {
  ...
  sock_t sock, ret = (sock_t) -1; // Modification for mmex compiler warning
...
int mg_websocket_write(struct mg_connection* conn, int opcode,
    ...
    if (data_len < 126) {
      ...
      copy[1] = (unsigned char) data_len; // Modification for mmex compiler warning
...
static void handle_put(struct connection *conn, const char *path) {
  ...
  if ((rc = put_dir(path)) == 0) {
    ...
    send_http_error(conn, 411);
              ...
              _open(path, O_RDWR | O_CREAT | O_TRUNC, 0644)) < 0) {
    send_http_error(conn, 500);
  } else {
...
static void open_local_endpoint(struct connection *conn) {
  ...
  } else if ((conn->endpoint.fd = _open(path, O_RDONLY | O_BINARY)) != -1) {
...
unsigned int mg_poll_server(struct mg_server *server, int milliseconds) {
  ...
  sock_t max_fd = (sock_t)-1; // Modification for mmex compiler warning
...
void mg_destroy_server(struct mg_server **server) {
  ...
  if (server != NULL && *server != NULL) {
    ...
    LINKED_LIST_FOREACH(&(*server)->active_connections, lp, tmp) {
      // Start Modificiations (free saved path_info from mg_strdup)
      free(((struct connection *)lp)->path_info);
      // End Modifications (free saved path_info from mg_strdup)
      free(LINKED_LIST_ENTRY(lp, struct connection, link));
    }
    LINKED_LIST_FOREACH(&(*server)->uri_handlers, lp, tmp) {
      // Start Modificiations (free saved handler uri)
      free(((struct uri_handler *)lp)->uri);
      // End Modifications (free saved handler uri)
    ...
    // End Modifications (free saved options)
    free(*server);
    *server = NULL;
  }
}
